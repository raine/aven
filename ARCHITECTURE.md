...
`src/sync/encrypted.rs` wires the existing host APIs to `aven sync setup`,
`sync invite`, `sync join`, and encrypted `aven sync` and `sync status`.
Behavior switches only on nonsecret database pins: a seed genesis pin or
enrollment pin makes a database encrypted, and plaintext sync keeps refusing
it. The server is the locator bound into protected enrollment identity, never
config, `AVEN_SYNC_SERVER` or `--server`. Setup reruns resume: once a sealed
...
re-executed test-binary workers against an `aven server --encrypted` worker.
Focused tests: `cargo test --lib 'sync::encrypted::'`.
...
| Change encrypted sync commands or encrypted server launch | `src/sync/encrypted.rs`, `src/sync/encrypted/invitation.rs`, `src/sync/server.rs` | `src/cli/sync.rs`, `src/lib.rs` sync dispatch, protected association helpers in `src/protected_local_keys/peer.rs`, server setup persistence in `crates/aven-core/src/sync/seed_claim/persistence.rs` | `cargo test --lib 'sync::encrypted::'`, `cargo test -p aven-core --lib 'sync::seed_claim::tests::'` |
