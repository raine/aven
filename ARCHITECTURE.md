...
3. `App` holds TUI flow controllers and simple UI state. Focused `src/tui/app_*.rs` modules coordinate feature flows. `SearchController` in `src/tui/app_search.rs` owns live search preview state and worker lifecycle, while `UpdateController` owns update flow resources and state.
