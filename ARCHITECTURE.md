...
| Core consumer API | workspace discovery, task and recurrence creation and updates, queue capture, state-checked queue quick mutations, task soft deletion and restoration, conditional undo, workspace-scoped project browsing and task search, recurrence lifecycle and reports, conflict inspection and resolution, HTTP-client-independent sync sessions, and the versioned pairing invitation contract | `crates/aven-core/src/api.rs`, `crates/aven-core/src/pairing.rs`, `crates/aven-core/src/sync/session.rs`, `crates/aven-core/tests/consumer_api.rs` | `api::Store` wraps `Database` and exposes owned, platform-neutral consumer DTOs, typed error codes, recurrence-aware mutation routing, reconcile-then-query reports, and sync session creation for host HTTP adapters. `TaskCapture::source` records the calling platform without splitting the operation by platform. The pure pairing contract owns URI versioning, bounded payload limits, server URL validation, and secret redaction. SQLx handles, transactions, internal operation types, query implementation, schedule math, sync JSON, configuration, transport, and rendering remain behind or outside the facade as appropriate. |
| Pairing presentation | reusable pairing input resolution, invitation-to-QR presentation state, and secret-safe diagnostics | `src/pairing.rs`, `src/commands/pairing.rs`, `src/tui/app_pairing.rs`, `src/tui/ui/overlays/pairing.rs` | The root pairing resolver applies the shared server precedence and trimmed token semantics before `PairingPresentation` consumes `aven_core::api::PairingInvitation`. Presentation state retains only the display-safe server origin and `PairingQr`, whose compact half-block rows include the four-module quiet zone and are shared by CLI and TUI renderers. CLI owns terminal detection, width checks, ANSI policy, and explicit local clipboard invitation output through `sync pair --copy`. Clipboard mode bypasses QR construction and keeps helper output out of diagnostics. The TUI modal holds the presentation behind an `Arc`, wraps server and private-network guidance, and renders a command-line fallback instead of a partial QR. Escape and a standard outside click dismiss it. Debug and error output omit tokens, QR rows, secret URL paths, and invitation URIs. |
...
Setup remains explicit library construction, not a CLI promise of ordinary sync.
...
Encrypted-tail requests fail closed, including retries from old sessions.
...
and unresolved-disclosure fences remain intact. Installed does not mean encrypted
tail-ready; no ordinary encrypted sync or shipping setup command is exposed.
...
ciphertext or rereading live domain state. Client validation compares decrypted
...
require that head to match the supported publication or first-peer chain. A historical outcome
...
The isolated seed HTTP router consumes these APIs; shipping plaintext sync does not.
...
membership/PoP signatures, HPKE PSK request/grant and exact bootstrap trust handoff.
Genesis/publication bytes remain unchanged. `peer/persistence.rs` derives current
authority from the verified chain for both independent credentials, atomically
commits admission/head, and retains one invitation/request/outcome slot. Unknown
heads fail closed; they are not evidence of removal. Immutable old outcomes do
not authorize historical credentials. The one-slot, first-peer-only subset is an
internal implementation boundary, not a permanent two-device product limit.

`src/protected_local_keys/peer.rs` owns append-only protected identity, recipient
binding, exact candidate/request, dispatch/disclosure marker, verified key coverage
and checkpoint. SQLite retains only loss-detection commitments, never keys. Fresh
target eligibility is preflighted under the installation interlock before creating
the irreversible denial marker, then rechecked in the pin transaction. Later local
edits can block completion but are never erased. Ordinary sync/replacement fences
and data-only exports also cover enrollment-bound databases.

`src/peer_enrollment_http.rs` is a separate bounded router/client for internal
construction. The unauthenticated mailbox exposes only public/encrypted enrollment
artifacts. Peer bearer authentication under the current head permits the published
descriptor read needed to finish trust verification, not staging/chunk/image reads.
There is no fresh-state installation or ordinary encrypted tail. No CLI pairing,
removal, cancellation, rotation or invitation reset is offered. Failed/expired or
uncertain attempts retain their slot and exact authority rather than rebind/reseal.
`ProtectedLocalKeyStore::enrollment_readiness` exposes the negative unresolved
key-disclosure fence that future encrypted dispatch must consult in addition to
current membership, installed state and key coverage. Enrolled does not mean synced.

Focused tests: `cargo test -p aven-core --lib 'sync::seed_claim::peer::tests::'`
and `cargo test --lib 'peer_enrollment_http::tests::'`, followed by seed HTTP,
protected adoption and plaintext loopback regressions. Process workers exit at
protected binding/candidate/send/verification checkpoints without destructors;
the server tests exercise transactional faults and independent-pool races. These
do not establish power-loss durability, production Keychain or multi-device
rotation behavior. Generalized enrollment must preserve these immutable records
and extend the same current-head validator, not add a parallel authorization ACL.
...
4. `crates/aven-core/src/sync/session.rs` first validates the server pin and discovers compatibility through an authenticated empty `/sync` request with maximal cursor. Mismatch-discovered supported candidates require one exact confirmation probe. Discovery never applies its cursor or establishes replica behavior. Only then does the session prepare bounded metadata and attachment exchanges. Persistence selects an ordered push prefix within count and complete serialized request byte limits before attachment planning. A first pending change that cannot fit alone produces a preparation error rather than an empty page. Attachment additions probe server inventory, upload missing content-addressed objects under lifecycle leases, confirm admission, and then send metadata. Missing local objects are downloaded after metadata application. Each round is bounded by object and byte budgets.
5. `src/sync/client.rs` executes core-owned retry and timeout guidance with Reqwest, replays the outstanding prepared exchange byte-for-byte, and returns the HTTP status, relevant headers, and decoded response bytes to the core session. Repeated preparation returns the same outstanding request until the page is accepted or explicitly failed.
6. `crates/aven-core/src/sync/wire.rs` owns the active server protocol constant, request limits, cursor validation, response validation, and daemon sync budget constants. `sync/protocol.rs` owns the maintained client baseline, immutable operation and closed-value contracts, discovery envelope and mismatch parser, and persisted replica behavior. Production currently supports protocol 18; illustrative later generations exist only in core tests.
7. `src/sync/server.rs` owns Axum presentation and authentication, while core sync persistence validates accepted operations and constructs bounded response pages. Server attachment admission reads, hashes, and decodes immutable object content before acquiring the writer gate, then rechecks inventory availability and metadata, object existence, and reservation or live-reference admission inside atomic sequence assignment. Background maintenance periodically runs the established attachment prune path independently of sync requests.
8. The server assigns strictly increasing `server_seq` values to accepted changes, reuses existing values for duplicate pushed change IDs, returns one bounded pull page, and sets `has_more` when rows remain after the page.
9. The core session decodes and validates each response before applying it. Response changes must fit the requested pull limit, have strictly increasing `server_seq` values after the cursor, match the response cursor, and include one push acknowledgement for each pushed change.
10. Remote apply checks the prepared cursor and previous replica behavior, then establishes the selected protocol and clears compatibility blocking in the same accepted-page transaction. Pending operations retain their identities and canonical meaning across cutovers. Remote apply updates local tables transactionally, verifies duplicate change IDs against canonical operation equality, records scalar and recurrence-aware conflicts for version mismatches, applies push acknowledgements, and advances `sync_cursor` only when the database cursor still matches the prepared request context. Recurrence create, template, projection, outcome, pause, lifecycle, and stop payloads carry canonical schedule and identity values, so apply never derives them from the receiver's zone or wall clock. The apply transaction reconciles every affected series after the complete page is applied.
11. The CLI adapter drives the core session until local unsynced backlog and remote `has_more` are both empty.
12. `src/status.rs` combines observational core persistence reads, application configuration, local attachment availability, and host service inspection into versioned privacy-safe sync and daemon reports. Status rendering stays in `src/commands/status.rs` and performs no transport or service action.
13. `src/daemon.rs` drives a session with `DAEMON_SYNC_PAGE_BUDGET`; incomplete rounds report counts, cursor, completion, and page count, then schedule prompt follow-up work. Contended rounds reschedule without transport failure output or sync persistence error state.
...
- Consumers create and parse pairing invitations through `PairingInvitation::new` and `PairingInvitation::decode` in the core pairing API. The contract keeps format constants private, bounds encoded input before base64 allocation, and exposes direct typed errors. Configuration reads, QR encoding and rendering, transport, and UI presentation stay in host layers. Invitation values and errors render only the server origin, never the auth token, secret URL path, or complete encoded payload. `api::Error` remains the error type for `Store` operations.
...
- Keep release discovery, cache policy, sync compatibility checks, install ownership, archive verification, and executable replacement in `src/update/`. A direct update that changes or cannot identify its sync protocol must check the configured server before installation and require separate confirmation when compatibility is not established. Keep CLI update presentation in `src/commands/self_update.rs` and TUI update flow state, cancellation, and overlay coordination in `src/tui/app_update.rs`. Never replace package-manager-owned executables or request elevated privileges.
...
| Add or change the pairing invitation contract | `crates/aven-core/src/pairing.rs` | `crates/aven-core/src/api.rs` re-exports and `crates/aven-core/src/sync/wire.rs` URL rules | `cargo test -p aven-core pairing::tests::` |
| Add or change pairing input resolution, QR construction, or CLI presentation | `src/pairing.rs` | `src/config/paths.rs`, `src/commands/pairing.rs`, `src/cli/sync.rs`, standalone classification in `src/lib.rs`, `src/tui/store/config.rs`, and the core contract through `aven_core::api` | `cargo test --lib 'config::paths::tests::'`, `cargo test --lib 'pairing::tests::'`, `cargo test --lib 'commands::pairing::tests::'`, and `cargo test --test cli_pairing` |
| Add or change TUI pairing presentation | `src/tui/ui/overlays/pairing.rs` | `src/tui/overlay/state.rs`, `src/tui/overlay/view.rs`, standard modal routing in `src/tui/overlay/mouse.rs`, `src/tui/app_pairing.rs`, `src/tui/store/config.rs`, and command scope in `src/tui/event/catalog.rs` and `src/tui/event/action.rs` | `cargo test --lib 'tui::ui::overlays::tests::pairing_overlay::'`, `cargo test --lib 'tui::overlay::mouse::tests::pairing_'`, and `cargo test --lib 'tui::app::tests::command_and_config_overlays::pairing::'` |
...
| Change fixed first-peer enrollment | `crates/aven-core/src/sync/seed_claim/peer.rs`, `peer/persistence.rs`, `src/protected_local_keys/peer.rs`, `src/peer_enrollment_http.rs` | current-head authorization in bootstrap publication, protected source/adoption, fresh-target and replacement fences, data-only export | `cargo test -p aven-core --lib 'sync::seed_claim::peer::tests::'`, `cargo test --lib 'peer_enrollment_http::tests::'`, seed HTTP/protected adoption tests and plaintext loopback regression |
