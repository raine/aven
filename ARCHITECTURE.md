...
- External mobile consumers create and parse the token-based pairing invitations of the unencrypted protocol through `PairingInvitation::new` and `PairingInvitation::decode` in the core pairing API; this workspace does not use them. Device invitations for encrypted sync are `aven://pair/v2/` values owned by `src/sync/encrypted/invitation.rs`. The contract keeps format constants private, bounds encoded input before base64 allocation, and exposes direct typed errors. Configuration reads, QR encoding and rendering, transport, and UI presentation stay in host layers. Invitation values and errors render only the server origin, never the auth token, secret URL path, or complete encoded payload. `api::Error` remains the error type for `Store` operations.
