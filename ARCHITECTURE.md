...
| iOS app host | minimal app bundle, serial Rust worker adaptation, durable Application Support path selection, XCTest smoke proof, and simulator and device orchestration | `ios/AvenIOS.xcodeproj`, `ios/AvenIOS/`, `ios/AvenIOSTests/`, `ios/run-host-proof`, `justfile` | The `AvenIOS` scheme consumes the local `AvenUniFFI` product and targets arm64 iOS 17.0. Every synchronous facade call runs through `RustWorker`; Swift selects the app-container path, while Rust opens SQLite and runs migrations. `just ios-host-proof` rebuilds package artifacts, compiles both SDK destinations, tests, installs, and launches the proof harness. Generated Apple artifacts remain under ignored `ios/Generated/`. |
