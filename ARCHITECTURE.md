...
3. `crates/aven-core/src/sync/session.rs` prepares at most `MAX_PUSH_BATCH` unsynced rows, the current `sync_cursor` as `after`, and `MAX_PULL_BATCH` as `pull_limit`. It serializes and optionally compresses the request into an HTTP-client-independent method, URL, headers, body, and opaque request context.
