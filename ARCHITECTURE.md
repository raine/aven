...
| Recurrence domain | validated series IDs, fixed calendar rules, Monday-anchored schedule expansion, series-zone slot timing, DST resolution, deterministic occurrence identity, and local aggregate operations | `crates/aven-core/src/recurrence.rs`, `crates/aven-core/src/recurrence/`, `crates/aven-core/src/operations/recurrence.rs` | Schedule math and occurrence identity stay pure and device-independent. Core recurrence operations atomically own template snapshots, projection reconciliation, outcomes, lifecycle, stable refs, and eligible immediate undo. Task status and deletion mutations route through the recurrence aggregate before generic scalar writes. Persisted recurrence read models belong to core rather than CLI or TUI adapters. |
...
| Add or change recurrence schedule math, deterministic occurrence identity, or local aggregate behavior | `crates/aven-core/src/recurrence/`, `crates/aven-core/src/operations/recurrence.rs` | `crates/aven-core/src/recurrence.rs`, `crates/aven-core/src/mutation.rs`, `crates/aven-core/src/operations/tasks.rs`, `crates/aven-core/src/undo.rs`, recurrence read models, and sync validation when persisted behavior is involved | `cargo test -p aven-core operations::recurrence::tests` for aggregate behavior and `cargo test -p aven-core recurrence` for schedule and identity behavior |
