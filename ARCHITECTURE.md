...
| Image attachments | content-addressed object storage, image validation and optimization, attachment metadata, lifecycle policy, leases, atomic task attachment mutations, and attachment read models | `crates/aven-core/src/attachments/`, `crates/aven-core/src/operations/attachments.rs`, `crates/aven-core/src/operations/tasks.rs`, `crates/aven-core/src/task_enrichment.rs` | Core owns durable attachment state and object lifecycle. Root modules own filesystem input and output, terminal preview rendering, configuration, and transport. |
...
- Attachment object state: `blob_inventory`, `blob_lifecycle`, `blob_leases`, `blob_upload_reservations`, `server_blob_references`, and `server_task_tombstones`; encoded image bytes live in content-addressed files under the configured blob directory.
