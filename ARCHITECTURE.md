...
| Device invitation presentation | invitation-to-QR presentation state shared by CLI and TUI | `src/pairing.rs`, `src/sync/encrypted.rs`, `src/tui/app_pairing.rs`, `src/tui/ui/overlays/pairing.rs` | `PairingPresentation` renders an already encoded `aven://pair/v2/` device invitation into a display-safe server origin and `PairingQr`, whose compact half-block rows include the four-module quiet zone. `aven sync invite` prints the invitation text on standard output and the QR on an interactive standard error. The TUI Add device action (`:add-device` or the Sync dialog) reuses `sync::encrypted::create_invitation` and `await_admission` through `InviteController`, shows the QR overlay, and keeps waiting after the overlay closes. Debug and error output omit QR rows and invitation text. The controller keeps the waiting invitation's text in zeroizing memory only so an explicit `c` in the overlay can copy it; nothing else writes it to the clipboard. |
...
The CLI commands wrap reusable host functions that return typed results and
take no terminal input: `run_setup`, `run_join`, `setup_preview`,
...
The TUI Sync dialog (`:sync`, `C s`, header click; `S` still syncs at once)
presents these functions. `src/tui/sync_operations.rs::SyncOperations` owns
the single running setup, join, device listing, removal or removal finish, its
latest stage and result, and the last server-verified device list, so closing
the overlay never cancels work; dropping the TUI aborts it and the engine's
durable state resumes it later. Manual sync and new invitations wait while an
operation runs. `src/tui/app_sync_dialog.rs` coordinates pages, validation with
the engine's invitation parsers, confirmation and polling;
`src/tui/overlay/sync_dialog.rs` holds page state, the never-rendered
`SecretText` invitation input and key handling; `src/tui/ui/overlays/sync_dialog.rs`
builds one layout for rendering and mouse hit testing. `src/tui/sync_errors.rs`
maps engine error codes to plain messages and keeps the error chain as
details, never inferring removal from a refusal. While joining awaits task
installation, creation actions that would make the target nonempty are paused.
Focused tests: `tui::app::tests::sync::`, `tui::overlay::sync_dialog::`,
`tui::ui::overlays::tests::sync_dialog_overlay::`, `tui::sync_operations::`
and `tui::sync_errors::`.
