...
| macOS Swift local proof | SwiftPM executable fixture, blocking-worker adaptation, typed mapping checks, database reopen proof, and generated binding consumption | `experiments/aven-swift-cli/Package.swift`, `experiments/aven-swift-cli/Sources/`, `experiments/aven-swift-cli/Tests/`, `justfile` | The proof calls the production UniFFI facade from a dedicated Dispatch queue through checked continuations. Rust owns SQLite, SQL, migrations, local domain behavior, and sync bytes. Generated bindings and the host static archive live under the package's ignored `Generated/` directory. `just swift-local-proof` regenerates, builds, tests, and runs the fixture. |
...
- Keep `experiments/aven-swift-cli` limited to local fixture orchestration, typed Swift mapping checks, and dedicated worker-executor adaptation. The Swift package has no SQLite or HTTP dependency, executes no SQL, and treats prepared sync bodies as opaque bytes. Generated bindings and the host static archive are reproducible outputs of `just uniffi-swift` and remain ignored.
...
| Add or change the macOS Swift local proof | `experiments/aven-swift-cli/Package.swift`, `experiments/aven-swift-cli/Sources/` | `experiments/aven-swift-cli/Tests/`, `crates/aven-uniffi/src/lib.rs`, generated artifact staging in `just uniffi-swift` | `just swift-local-proof` and focused `aven-uniffi` facade tests |
...
- `just swift-local-proof`: regenerate the host artifacts, run `swift build` and `swift test`, then execute the local macOS Swift fixture.
