...
- Keep TUI attachment placeholders metadata-backed. `src/task_enrichment.rs` loads attachment metadata, `src/tui/markdown.rs` parses attachment image blocks, and `src/tui/ui/detail.rs` applies the task detail quote rail. `src/tui/inline_images.rs` owns terminal preview backend detection and escape construction. Placeholders remain canonical when previews are disabled, unsupported, hidden by overlays, missing local bytes, deleted metadata, missing metadata, or tmux auto mode.
