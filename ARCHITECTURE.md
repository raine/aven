...
2. Pure routers in `src/tui/input/key.rs` and `src/tui/input/mouse.rs` translate terminal events into semantic actions or feature-owned input events. Capturing overlays receive their focused events after top-level routing.
...
- Structured mutation reports drive messages and selection restoration. TUI refreshes build a complete replacement projection and publish it only after every query succeeds. A committed-refresh error closes completed edit flows so stale input cannot duplicate a mutation.
...
- `src/tui/app_update.rs` coordinates update presentation and overlays. `src/update/` owns release discovery, verification, install classification, and executable replacement.
...
- Backup and portability workflows: `src/commands/data_safety/mod.rs` owns filesystem selection and presentation, while `crates/aven-core/src/data_safety.rs` and `crates/aven-core/src/data_safety/` own scans, validation, import, integrity, and backup mechanics. Portable exports include recurrence series, template labels, sparse occurrences, pause intervals, generalized conflicts, and generalized field versions. Older task-only exports create no recurrence linkage. SQLite backup and restore preserve every table and archived task row through SQLite's backup mechanism.
...
- Use `crates/aven-core/src/query.rs`, `crates/aven-core/src/query/`, and core enrichment helpers for read models. Recurrence-aware report entry points reconcile before querying, cap candidates at 256, and may commit one atomic projection transaction per changed series after a slot boundary. Render-only adapters consume the returned report state.
...
- Resolve raw key and mouse input in `src/tui/input/` against shortcut, capture, and rendered view-model state. Keep routers free of persistence and platform effects. Execute semantic actions through the exhaustive table in `src/tui/input/action.rs`.
