...
7. `src/tui/ui.rs` and `src/tui/ui/` render state. `DetailDocument` in `src/tui/ui/detail.rs` owns each frame's semantic task-detail projection, including wrapped Markdown, sticky and body lines, interactive rows, selection mapping, scroll bounds, target reveal positions, hit testing, and inline-image placement. Rendering stores the document in `WidgetState`, and detail input reuses that frame-owned value until the next draw. Linked detail navigation loads an exact workspace-scoped task through `TuiStore`, independently of the active list filters, and preserves the originating view and detail UI state in bounded history. Add-task composer image paste records structured pending attachments separately from its text fields. `src/tui/app_lifecycle.rs` polls attachment completion, emits previews after frame draws, owns placement cleanup and terminal writes, and waits for in-flight attachment commits during orderly shutdown. Rendering code does not touch the database or filesystem.
...
- Keep TUI database access in `src/tui/store/`; keep `src/tui/ui/` rendering-only. `src/tui/store/launch.rs` is the sole database-backed boundary from raw TUI CLI arguments to resolved launch state. Durable local TUI values use owned methods backed by `crates/aven-core/src/local_state.rs`. Keep tabular task-list view modeling and hit testing in `src/tui/ui/task_list/`. Keep column grouping and navigation in `src/tui/columns.rs`, and use one shared layout from `src/tui/ui/columns.rs` for column rendering and hit testing. Resolve selected and visible marked tasks through `TaskSelection`, pass that value to the one batch-capable store policy for each mutation family, and retain it for the lifetime of edit overlays and confirmations. `DetailDocument` owns the task-detail projection for rendering, scrolling, focus traversal, selection, hit testing, and image placement. Pointer handling uses the document stored by the latest frame rather than rebuilding Markdown projection.
- Keep TUI attachment sections metadata-backed for committed attachments and app-state-backed for local preparation. `crates/aven-core/src/task_enrichment.rs` loads ordered live attachment metadata, and `src/tui/attachment_controller.rs` owns pending and failure state without publishing a hash. Attachment focus depends on live image metadata and local byte availability, independently of terminal graphics support. `src/tui/preview_controller.rs` owns bounded thumbnail work, caching, leases, retries, and stale-result rejection. `src/attachments/export.rs` owns secure leased copies for external viewers, while `src/tui/platform.rs` owns direct platform launcher commands.
...
| Change TUI detail rendering or interaction projection | `src/tui/ui/detail.rs` (`DetailDocument`, `DetailRenderContext`) | `src/tui/ui.rs`, `src/tui/app_dispatch.rs`, `src/tui/navigation.rs`, inline-image and selection state | `cargo test tui::ui::detail`; focused app filters `detail_scroll`, `detail_drag`, `detail_attachment`, `detail_child`, and `stable_frame_mouse_movement_reuses_detail_document` |
...
| Add or change a TUI action | `src/tui/event/catalog.rs`, `src/tui/app_dispatch.rs`, focused `src/tui/app_*.rs` modules | flow helpers, overlays, store module, core operation and undo mode, `src/tui/natural_add_runtime.rs` for natural-add worker setup | `src/tui/app_tests.rs`, `src/tui/store/tests.rs`, overlay module tests |
