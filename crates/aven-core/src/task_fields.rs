...
impl TaskField {
    pub const VERSIONED: [TaskField; 9] = [
        TaskField::Title,
    ...
    pub fn parse_or_unknown(field: &str) -> Result<Self> {
        Self::parse(field).ok_or_else(|| anyhow!("error unknown-field field={field}"))
    }

    pub fn parse_for_sync(field: &str) -> Result<Self> {
    ...
    pub fn updates_queue_activity(self) -> bool {
        matches!(self, Self::Status | Self::Priority)
    }

    pub fn scalar_payload(
        self,
