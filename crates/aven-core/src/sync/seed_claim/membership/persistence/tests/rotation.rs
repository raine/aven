...
#[tokio::test]
async fn admission_only_store_is_refused_without_changing_source_or_history() {
    let f = Fixture::new().await;
    let (m, d, peer, receipt) = pair(&f).await;
    ...
    sqlx::query("DELETE FROM server_membership_transitions")
        ...
        .await
        .unwrap();
    sqlx::query("INSERT INTO server_membership_admissions(sequence,handle,record) VALUES(2,?,?)")
    ...
    let reopened = Database::open(&f.dir.path().join("server.db"))
        .await
        .unwrap();
    let a = auth(&f, m.head(), peer.device(), peer.bearer());
    assert_eq!(
        reopened
    ...
    let preserved: Vec<u8> =
        sqlx::query_scalar("SELECT record FROM server_membership_admissions WHERE sequence=2")
            .fetch_one(&mut *reopened.acquire_reader().await.unwrap())
            .await
            .unwrap();
    assert_eq!(preserved, receipt);
    let source = Database::open(&f.dir.path().join("source.db"))
        .await
        .unwrap();
    let tables: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='tasks'",
    )
    .fetch_one(&mut *source.acquire_reader().await.unwrap())
    .await
    .unwrap();
    assert_eq!(tables, 1);
}
