...
impl Database {
    pub async fn display_ref_context(
        &self,
...
impl DisplayRefContext {
    ...
    pub fn display_suffix(&self, workspace_id: &WorkspaceId, id: &TaskId) -> String {
        let ids = self
            .task_ids_by_workspace
            .get(workspace_id)
            .map(Vec::as_slice)
