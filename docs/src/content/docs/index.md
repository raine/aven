...
- **The same task store has many entrypoints.** Tasks should be reachable from the laptop TUI, agent CLI, terminal capture flows, sync clients, and integrations that talk to the local or synced task store. Run the sync server on a Raspberry Pi or home server, then sync devices through a VPN or private network.
