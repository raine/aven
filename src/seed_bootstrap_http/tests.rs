...
type FrozenPublicationRow = (String, Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>);
type FrozenRecordRow = (String, Vec<u8>, i64, Vec<u8>);

async fn frozen_rows(
    conn: &mut sqlx::SqliteConnection,
) -> (FrozenPublicationRow, Vec<FrozenRecordRow>) {
...
/// Rewrites the current frozen package into the storage layout that preceded the
/// single record table, with every metadata column derived from the exact bytes.
async fn restore_previous_package_storage(conn: &mut sqlx::SqliteConnection) {
    use sha2::{Digest, Sha256};
    let ((candidate, descriptor, data, prefix, images), records) = frozen_rows(conn).await;
    let (image_sha, classification): (String, String) = sqlx::query_as(
        "SELECT sha256, classification FROM local_shared_capture_images
         WHERE classification != 'unavailable'",
    )
    .fetch_one(&mut *conn)
    .await
    .unwrap();
    sqlx::raw_sql(concat!(
        ...
        include_str!(
            "../../crates/aven-core/migrations/20260922062906_encrypted_local_shared_capture_package.sql"
        ),
        include_str!(
            "../../crates/aven-core/migrations/20260922073647_encrypted_local_shared_capture_images.sql"
        ),
    ...
    .await
    .unwrap();
    let artifact = |component: &str| {
        let chunks = records
            .iter()
            .filter(|(c, _, _, _)| c == component)
            .map(|(_, _, _, record)| record.clone())
            .collect::<Vec<_>>();
        let total = chunks.iter().map(|r| r.len() as i64 - 222).sum::<i64>();
        let aggregate = Sha256::digest(chunks.concat()).to_vec();
        (total, chunks, aggregate)
    };
    let (state_total, state, state_aggregate) = artifact("state");
    let (manifest_total, manifest, manifest_aggregate) = artifact("manifest");
    let (image_total, image, image_aggregate) = artifact("image");
    let object = records
        .iter()
        .find(|(c, _, _, _)| c == "image")
        .unwrap()
        .1
        .clone();
    assert!(
    ...
    .bind(&candidate)
    .bind(&descriptor[7..39])
    .bind(&descriptor[71..103])
    .bind(state_total)
    .bind(state.len() as i64)
    .bind(state_aggregate)
    .bind(manifest_total)
    .bind(manifest.len() as i64)
    .bind(manifest_aggregate)
    .execute(&mut *conn)
    .await
    .unwrap();
    sqlx::query("INSERT INTO local_shared_capture_publication VALUES (?, ?, ?, ?, ?)")
        .bind(&candidate)
        .bind(&descriptor)
        .bind(&data)
        .bind(&prefix)
        ...
        .await
        .unwrap();
    for (class, chunks) in [(1, &state), (2, &manifest)] {
        for (index, record) in chunks.iter().enumerate() {
            sqlx::query(
                "INSERT INTO local_shared_capture_package_chunks VALUES (?, ?, ?, ?, ?, ?)",
            )
            .bind(&candidate)
            .bind(class)
            .bind(index as i64)
            .bind(record.len() as i64)
            .bind(Sha256::digest(record).to_vec())
            .bind(record)
            .execute(&mut *conn)
            .await
            .unwrap();
    ...
    sqlx::query("INSERT INTO local_shared_capture_package_images VALUES (?, ?, ?, ?, ?, ?, ?)")
        .bind(&candidate)
        .bind(&image_sha)
        .bind(&classification)
        .bind(&object)
        ...
        .bind(image_aggregate)
        .execute(&mut *conn)
        .await
        .unwrap();
    for (index, record) in image.iter().enumerate() {
        sqlx::query(
            "INSERT INTO local_shared_capture_package_image_chunks VALUES (?, ?, ?, ?, ?, ?)",
        )
        .bind(&candidate)
        .bind(&object)
        .bind(index as i64)
...
#[tokio::test]
async fn frozen_package_storage_migration_preserves_sealed_upload_and_adoption() {
    let root = tempfile::tempdir().unwrap();
    let (db, store, seed, package) = fixture(root.path()).await;
    ...
    let intent = store
        ...
        .to_vec();
    let mut conn = aven_core::test_support::acquire(&db).await.unwrap();
    let before = frozen_rows(&mut conn).await;
    restore_previous_package_storage(&mut conn).await;
    sqlx::migrate!("crates/aven-core/migrations")
        .run(&mut *conn)
        .await
        .unwrap();
    assert!(frozen_rows(&mut conn).await == before);
    let (state, previous_tables): (String, i64) = sqlx::query_as(
        "SELECT (SELECT state FROM local_seed_publication_intent),
                (SELECT count(*) FROM sqlite_master
                 WHERE name IN ('local_shared_capture_packages',
                                'local_shared_capture_package_chunks',
                                'local_shared_capture_package_images',
                                'local_shared_capture_package_image_chunks'))",
    )
    .fetch_one(&mut *conn)
    .await
    .unwrap();
    assert_eq!((state.as_str(), previous_tables), ("sealed", 0));
    drop(conn);
    let path = db.path().to_path_buf();
    drop(db);
    let db = Database::open(&path).await.unwrap();

    let (_, resumed, upload) = store.seed_http_inputs(&db).await.unwrap();
    assert_eq!(resumed.protected_storage_bytes(), intent);
    assert!(upload.unwrap() == package);
    let server = Database::open(&root.path().join("server.sqlite"))
        .await
        .unwrap();
    let (http, task) = serve(server).await;
    ...
    let adopted: (String, i64) = sqlx::query_as(
        "SELECT (SELECT state FROM local_seed_publication_intent),
                (SELECT count(*) FROM local_e2ee_image_objects WHERE origin = 'bootstrap')",
    )
    .fetch_one(&mut *conn)
    .await
    .unwrap();
    assert_eq!((adopted.0.as_str(), adopted.1), ("adopted", 1));
    task.abort();
}
