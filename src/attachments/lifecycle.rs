...
pub(crate) const DEFAULT_LOCAL_GRACE: Duration = Duration::from_secs(7 * 24 * 60 * 60);
pub(crate) const DEFAULT_ORIGINAL_QUOTA_BYTES: i64 = 10 * 1024 * 1024 * 1024;
pub(crate) const DEFAULT_PREVIEW_QUOTA_BYTES: u64 = 512 * 1024 * 1024;
pub(crate) const DEFAULT_MAINTENANCE_LIMIT: usize = 128;
...
pub(crate) struct LifecyclePolicy {
    ...
    pub(crate) quota_bytes: i64,
    pub(crate) preview_quota_bytes: u64,
    pub(crate) maintenance_limit: usize,
}
...
pub(crate) struct LifecycleReport {
    pub(crate) referenced: ByteCount,
    pub(crate) protected: ByteCount,
    pub(crate) grace_period: ByteCount,
...
pub(crate) async fn reconcile_liveness(
...
pub(crate) async fn release_lease(conn: &mut SqliteConnection, lease_id: &str) -> Result<()> {
...
pub(crate) async fn local_unique_bytes(conn: &mut SqliteConnection) -> Result<i64> {
...
pub(crate) async fn ensure_local_capacity(
    conn: &mut SqliteConnection,
    blob_dir: &Path,
    sha256: &str,
    byte_size: i64,
...
pub(crate) async fn reconcile_trash(conn: &mut SqliteConnection, blob_dir: &Path) -> Result<()> {
...
pub(crate) async fn reconcile_staging(blob_dir: &Path) -> Result<ByteCount> {
...
pub(crate) async fn reconcile_orphan_objects(
    conn: &mut SqliteConnection,
    blob_dir: &Path,
    grace: Duration,
    clock: &dyn Clock,
...
pub(crate) fn prune_preview_cache(blob_dir: &Path, quota: u64) -> Result<ByteCount> {
