...
use crate::refs::resolve_task_ref_in_workspace;
...
impl TuiLaunch {
    pub(crate) async fn resolve(
        ...
        let scope = match args.project.as_deref() {
            Some("") => inferred_existing_project_key_in_workspace(conn, &workspace.id)
                .await?
        ...
        let label = match args.label {
            Some(label) => {
                Some(ensure_label_exists_in_workspace(conn, &workspace.id, &label).await?)
            }
            None => None,
        };
...
mod tests {
    ...
    async fn resolves_composed_browse_state() {
        ...
        input.priority = Some(crate::choices::TaskPriority::High);
        input.add_task = true;
        input.natural = true;

        let launch = TuiLaunch::resolve(&mut conn, &Workspace::default(), input)
            .await
            .unwrap();
    ...
    async fn rejects_task_filters_for_recent_actions() {
        ...
        input.priority = Some(crate::choices::TaskPriority::Urgent);

        let error = TuiLaunch::resolve(&mut conn, &Workspace::default(), input)
            .await
            .unwrap_err();
