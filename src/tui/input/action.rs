...
impl App {
    pub(in crate::tui) async fn execute(&mut self, action: Action) -> Result<()> {
        ...
        match action {
            ...
            Action::BeginBrowseLabels => self.begin_browse_labels().await?,
            Action::BeginRenameLabel => self.begin_rename_label().await?,
            Action::BeginDeleteLabel => self.begin_delete_label().await?,
