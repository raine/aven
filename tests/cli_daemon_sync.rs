...
fn daemon_syncs_large_backlog_across_budgeted_rounds() {
    ...
    let incomplete = format!(
        "daemon-synced pushed={first_pushed} pulled=0 blob_uploaded=0 blob_downloaded=0 cursor={first_pushed} complete=false pages={DAEMON_SYNC_PAGE_BUDGET}"
    );
    let complete = format!(
        "daemon-synced pushed=1 pulled=0 blob_uploaded=0 blob_downloaded=0 cursor={task_count} complete=true pages=1"
    );
