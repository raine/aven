...
#[test]
fn bulk_update_preflight_queries_only_fields_each_task_will_change() {
    let env = TestEnv::new();
    let db = env.db("bulk-update-candidate-fields.sqlite");
    let task_ref = extract_ref(&ok(env.aven(
        &db,
        [
            "add",
    ...
    execute_sql(
        ...
        "INSERT INTO conflicts(workspace_id, task_id, field, local_value, remote_value,
         remote_change_id, variant_a, variant_b, created_at)
         SELECT workspace_id, id, 'title', title, 'remote title', 'irrelevant-title',
                title, 'remote title', 't'
         FROM tasks WHERE title = 'candidate fields'",
    );

    contains_all(
        &ok(env.aven(&db, ["bulk-update", "--all", "--set-priority", "high"])),
        &["matched=1 changed=1 would_change=1 unchanged=0"],
    );
    contains_all(&ok(env.aven(&db, ["show", &task_ref])), &["priority=high"]);

    execute_sql(
        &db,
        r#"INSERT INTO conflicts(workspace_id, task_id, field, local_value, remote_value,
         remote_change_id, variant_a, variant_b, created_at)
         SELECT workspace_id, id, 'metadata:FAKEFIELD0000001',
                '{"present":false,"value":""}',
                '{"present":true,"value":"remote"}',
                'absent-metadata', '', 'remote', 't'
         FROM tasks WHERE title = 'candidate fields'"#,
    );
    contains_all(
        &ok(env.aven(
            &db,
            ["bulk-update", "--all", "--metadata", "Brand_New=value"],
        )),
        &["matched=1 changed=1 would_change=1 unchanged=0"],
    );
    contains_all(
...
#[test]
fn bulk_update_preflight_preserves_field_error_order_for_labels_and_metadata() {
    let env = TestEnv::new();
    let db = env.db("bulk-update-candidate-order.sqlite");
    ok(env.aven(&db, ["label", "create", "bug"]));
    let task_ref = extract_ref(&ok(env.aven(
        &db,
        [
    ...
    execute_sql(
        ...
        r#"INSERT INTO conflicts(workspace_id, task_id, field, local_value, remote_value,
         remote_change_id, variant_a, variant_b, created_at)
         SELECT t.workspace_id, t.id, field,
                CASE WHEN field LIKE 'metadata:%'
                     THEN '{"present":true,"value":"old"}' ELSE 'local' END,
                CASE WHEN field LIKE 'metadata:%'
                     THEN '{"present":true,"value":"remote"}' ELSE 'remote' END,
                'ordered-' || field, 'local', 'remote', 't'
         FROM tasks t
         CROSS JOIN (
             SELECT 'priority' AS field
             UNION ALL SELECT 'labels'
             UNION ALL
             SELECT 'metadata:' || id FROM metadata_fields WHERE key = 'review_state'
         ) fields
         WHERE t.title = 'ordered conflicts'"#,
    );
    ...
    contains_all(&fail(env.aven(&db, args)), &["field=priority"]);
    execute_sql(
        &db,
        "UPDATE conflicts SET resolved = 1 WHERE field = 'priority'",
    );
    contains_all(&fail(env.aven(&db, args)), &["field=labels"]);
    execute_sql(
        &db,
        "UPDATE conflicts SET resolved = 1 WHERE field = 'labels'",
    );
    let metadata_error = fail(env.aven(&db, args));
    contains_all(
        &metadata_error,
        &["error bulk-update-conflicted-field", "field=metadata:"],
    );
    contains_all(
        &ok(env.aven(&db, ["show", &task_ref, "--full"])),
        &["status=inbox", "priority=none", "key=review_state", "old"],
    );
}
